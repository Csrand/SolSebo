...
const mockUser = {
  ...
  recovery_token: null,
  token_expires: null,
  verifyEmail: jest.fn(function () {
    this.status_validacao = true;
    this.verification_token = null;
    this.verification_token_expires = null;
  }),
  setRecoveryToken: jest.fn(function (token: string, expires: Date) {
    this.recovery_token = token;
    this.token_expires = expires;
  }),
  resetPassword: jest.fn(function (hash: string) {
...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('deve criar usuário e enviar email de verificação', async () => {
      ...
      const dto = { username: 'newuser', email: 'new@test.com', password: '123456', confirmPassword: '123456' };
      const result = await service.register(dto);

      expect(usersService.create).toHaveBeenCalled();
      expect(mailService.sendVerificationEmail).toHaveBeenCalledWith('new@test.com', expect.any(String));
    ...
    it('deve rejeitar username duplicado', async () => {
      ...
      const dto = { username: 'testuser', email: 'other@test.com', password: '123456', confirmPassword: '123456' };
  ...
  describe('verifyEmail', () => {
    it('deve ativar a conta com token válido', async () => {
      const user = { ...mockUser, verification_token_expires: new Date(Date.now() + 3600000) };
  ...
  describe('login', () => {
    ...
    it('deve rejeitar email não verificado', async () => {
      ...
      await expect(service.login({ email: 'test@test.com', password: '123456' })).rejects.toThrow('Email não verificado');
    });
  });
  ...
  describe('resetPassword', () => {
    it('deve redefinir senha com token válido', async () => {
      const user = { ...mockUser, token_expires: new Date(Date.now() + 3600000) };
      usersService.findByRecoveryToken.mockResolvedValue(user);

      const result = await service.resetPassword({ token: 'valid', newPassword: '654321', confirmNewPassword: '654321' });
    ...
    it('deve rejeitar token inválido', async () => {
      ...
      await expect(service.resetPassword({ token: 'invalid', newPassword: '654321', confirmNewPassword: '654321' })).rejects.toThrow(BadRequestException);
    });
  });
});
